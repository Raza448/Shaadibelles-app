 ...
 $scope.getCategories = function() {
    ...
    $http.get(window.remote + '/api/categories/' + $scope.id + '?access_token=' + $rootScope.user.accessToken)
      .then(function(res) {
        ...
        $scope.formData.keywords = res.data.keywords.toString();
        $scope.formData.created = res.data.created;
        $scope.formData.visible = res.data.visible;
        $scope.formData.parentId = res.data.parentId;
      });
